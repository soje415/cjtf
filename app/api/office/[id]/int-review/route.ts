...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (decision === 'reject') {
    ...
    }).eq('id', params.id).eq('status', 'PENDING_INT_SCREENING').select('id').maybeSingle()
  ...
  // Pass → admin approval. Conditional on the status we validated above so a
  // double submit can't duplicate the note or the Admin SMS.
  const { data: moved } = await service.from('office_registrations').update({
